...
function checkContrastOnDark(rgb: Rgb, d: HealthDiagnostic[]): number {
  ...
  if (ratio < 2.5) {
    d.push({
      ...
      severity: 'error',
      message: `contrast on dark background too low (${ratio.toFixed(1)}:1)`,
  ...
  if (ratio < 3.5) {
    d.push({
      ...
      severity: 'warning',
      message: `contrast on dark background is marginal (${ratio.toFixed(1)}:1)`,
...
function checkContrastOnLight(rgb: Rgb, d: HealthDiagnostic[]): number {
  ...
  if (ratio < 2.5) {
    d.push({
      ...
      severity: 'warning',
      message: `contrast on light background too low (${ratio.toFixed(1)}:1)`,
...
function checkTextOnColor(rgb: Rgb, d: HealthDiagnostic[]): number {
  ...
  if (best < 3) {
    d.push({
      ...
      severity: 'error',
      message: `neither white nor black text is readable on this color (best: ${best.toFixed(1)}:1)`,
  ...
  if (best < 4.5) {
    d.push({
      ...
      severity: 'warning',
      message: `text on this color has limited contrast (${best.toFixed(1)}:1)`,
...
function checkSaturation(s: number, d: HealthDiagnostic[]): number {
  if (s < 0.2) {
    d.push({
      id: 'saturation-low',
      severity: 'warning',
      message: `saturation too low (${(s * 100).toFixed(0)}%) — color appears grayish`,
  ...
  if (s > 0.95) {
    d.push({
      id: 'saturation-high',
      severity: 'info',
      message: `very high saturation (${(s * 100).toFixed(0)}%) — may cause eye strain`,
...
function checkLightness(l: number, d: HealthDiagnostic[]): number {
  if (l < 0.15) {
    d.push({
      ...
      severity: 'error',
      message: `color is too dark (L=${(l * 100).toFixed(0)}%) — invisible on dark mode`,
  ...
  if (l > 0.85) {
    d.push({
      ...
      severity: 'warning',
      message: `color is too light (L=${(l * 100).toFixed(0)}%) — weak on light mode`,
...
function checkSemanticConflict(
  h: number,
  s: number,
  d: HealthDiagnostic[]
): string | null {
  ...
  for (const range of SEMANTIC_RANGES) {
    if (h >= range.hueMin && h <= range.hueMax) {
      d.push({
        ...
        severity: 'warning',
        message: `hue (${h.toFixed(0)}°) overlaps with ${range.name} semantic color`,
        suggestion:
          'users may confuse accent with status colors — shift hue away from red/amber/green zones',
      })
      return range.name
    }
  }
