...
describe('MagneticButton', () => {
  ...
  it('has data-component attribute', () => {
    const { container } = render(<MagneticButton>Content</MagneticButton>)
    expect(
      container.querySelector('[data-component="magnetic-button"]')
    ).toBeInTheDocument()
  })

  it('applies transform on mouse move within radius', () => {
    const { container } = render(
      <MagneticButton strength={0.5} radius={200}>
        Content
      </MagneticButton>
    )
    const el = container.querySelector(
  ...
  it('resets transform when cursor is outside radius', () => {
    const { container } = render(
      <MagneticButton strength={0.3} radius={10}>
        Content
      </MagneticButton>
    )
    const el = container.querySelector(
  ...
  it('applies offset when cursor is within radius', () => {
    const { container } = render(
      <MagneticButton strength={1} radius={500}>
        Content
      </MagneticButton>
    )
    const el = container.querySelector(
  ...
  it('applies custom className', () => {
    const { container } = render(
      <MagneticButton className="extra">Content</MagneticButton>
    )
