...
describe('Masonry', () => {
  ...
  it('applies column-count via inline style', () => {
    const { container } = render(
      <Masonry columns={4}><div>A</div></Masonry>,
    )
    const el = container.querySelector('[data-component="masonry"]') as HTMLElement
    expect(el.style.columnCount).toBe('4')
  })

  it('applies custom gap', () => {
    const { container } = render(
      <Masonry gap={24}><div>A</div></Masonry>,
    )
    const el = container.querySelector('[data-component="masonry"]') as HTMLElement
