...
describe('SplitView', () => {
  ...
  it('applies custom className', () => {
    const { container } = render(<SplitView left={<div>L</div>} right={<div>R</div>} className="my-split" />)
  ...
  it('uses defaultSplit for initial pane widths', () => {
    const { container } = render(<SplitView left={<div>L</div>} right={<div>R</div>} defaultSplit={30} />)
    const panes = container.querySelector('[data-component="split-view"]')?.children
  ...
  it('supports callback ref', () => {
    ...
    render(<SplitView left={<div>L</div>} right={<div>R</div>} ref={(el) => { refVal = el }} />)
  ...
  it('handles resize via drag on the resize handle', () => {
    const { container } = render(<SplitView left={<div>L</div>} right={<div>R</div>} defaultSplit={50} />)
    const root = container.querySelector('[data-component="split-view"]') as HTMLDivElement
    // mock offsetWidth
    Object.defineProperty(root, 'offsetWidth', { value: 1000, configurable: true })

    const handle = container.querySelector('[data-component="resize-handle"]') as HTMLElement
  ...
  it('clamps split to minLeft', () => {
    const { container } = render(<SplitView left={<div>L</div>} right={<div>R</div>} defaultSplit={25} minLeft={20} />)
    const root = container.querySelector('[data-component="split-view"]') as HTMLDivElement
    Object.defineProperty(root, 'offsetWidth', { value: 1000, configurable: true })
  ...
  it('clamps split to maxRight (100 - minRight)', () => {
    const { container } = render(<SplitView left={<div>L</div>} right={<div>R</div>} defaultSplit={75} minRight={20} />)
    const root = container.querySelector('[data-component="split-view"]') as HTMLDivElement
    Object.defineProperty(root, 'offsetWidth', { value: 1000, configurable: true })
