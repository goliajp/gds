...
describe('StatGrid', () => {
  it('renders with data-component attribute', () => {
    ...
    expect(container.querySelector('[data-component="stat-grid"]')).not.toBeNull()
  })

  it('renders children', () => {
    render(<StatGrid><span>Child 1</span><span>Child 2</span></StatGrid>)
    expect(screen.getByText('Child 1')).toBeDefined()
    expect(screen.getByText('Child 2')).toBeDefined()
  })

  it('applies correct grid columns', () => {
    const { container } = render(<StatGrid columns={4}><div>A</div></StatGrid>)
