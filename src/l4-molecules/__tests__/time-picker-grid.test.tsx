...
describe('TimePickerGrid', () => {
  ...
  it('calls onHourClick when an hour button is clicked', async () => {
    ...
    render(<TimePickerGrid {...defaultProps} hours={[9, 14]} minutes={[]} onHourClick={onHourClick} />)
  ...
  it('calls onMinuteClick when a minute button is clicked', async () => {
    ...
    render(
      <TimePickerGrid {...defaultProps} hours={[]} minutes={[15, 30]} onMinuteClick={onMinuteClick} />,
    )
  ...
  it('highlights the selected hour', () => {
    render(<TimePickerGrid {...defaultProps} hours={[8, 9, 10]} minutes={[]} selectedHour={9} />)
  ...
  it('highlights the selected minute', () => {
    render(
      <TimePickerGrid {...defaultProps} hours={[]} minutes={[0, 15, 30]} selectedMinute={15} />,
    )
  ...
  it('all buttons have type="button"', () => {
    render(<TimePickerGrid {...defaultProps} hours={[1, 2]} minutes={[0, 30]} />)
    const buttons = screen.getAllByRole('button')
  ...
  it('applies solid styling when glass prop is false', () => {
    const { container } = render(<TimePickerGrid {...defaultProps} glass={false} />)
    const root = container.firstChild
