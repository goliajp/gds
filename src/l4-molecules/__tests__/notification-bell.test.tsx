...
describe('NotificationBell', () => {
  it('renders with data-component', () => {
    const { container } = render(<NotificationBell unread={0} onClick={() => {}} />)
    expect(container.querySelector('[data-component="notification-bell"]')).not.toBeNull()
  })

  it('shows unread count when > 0', () => {
    const { getByText } = render(<NotificationBell unread={5} onClick={() => {}} />)
    expect(getByText('5')).toBeDefined()
  })

  it('shows 99+ when unread > 99', () => {
    const { getByText } = render(<NotificationBell unread={100} onClick={() => {}} />)
  ...
  it('applies animate-pulse when unread > 0', () => {
    const { container } = render(<NotificationBell unread={3} onClick={() => {}} />)
  ...
  it('calls onClick when clicked', () => {
    const handler = vi.fn()
    const { container } = render(<NotificationBell unread={0} onClick={handler} />)
