...
describe('FieldGroup', () => {
  it('renders with data-component attribute', () => {
    const { container } = render(<FieldGroup><input /></FieldGroup>)
    expect(container.querySelector('[data-component="field-group"]')).not.toBeNull()
  })

  it('renders children', () => {
    render(<FieldGroup><span>Field content</span></FieldGroup>)
  ...
  it('renders title as legend when provided', () => {
    render(<FieldGroup title="Personal Info"><input /></FieldGroup>)
  ...
  it('does not render description when title is undefined even if description is set', () => {
    render(<FieldGroup description="desc"><input /></FieldGroup>)
    expect(screen.queryByText('desc')).toBeNull()
  })

  it('renders description when both title and description are provided', () => {
    render(<FieldGroup title="Info" description="Fill in details"><input /></FieldGroup>)
  ...
  it('does not render description when title is set but description is undefined', () => {
    const { container } = render(<FieldGroup title="Info"><input /></FieldGroup>)
  ...
  it('uses 2-column grid when columns=2', () => {
    const { container } = render(<FieldGroup columns={2}><input /></FieldGroup>)
    const grid = container.querySelector('.grid')
    expect(grid?.className).toContain('md:grid-cols-2')
  })

  it('uses 3-column grid when columns=3', () => {
    const { container } = render(<FieldGroup columns={3}><input /></FieldGroup>)
    const grid = container.querySelector('.grid')
    expect(grid?.className).toContain('lg:grid-cols-3')
  })

  it('applies custom className', () => {
    const { container } = render(<FieldGroup className="my-cls"><input /></FieldGroup>)
