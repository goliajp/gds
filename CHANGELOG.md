...
- `useFonts()` hook — auto-inject CJK fonts (Noto Sans SC/JP/KR) from Google Fonts CDN, no manual `<link>` tags needed

## [1.6.0] - 2026-03-29

### Added

- JSDoc comments on all props for Button, Input, DataTable, DataColumn, Dialog, Avatar, Card, GanttChart — IDE hover now shows descriptions
- theme.css: status colors (active/inactive/pending/draft), priority colors (critical/high/medium/low), action colors (create/update/delete), dot color, palette-0~9
- ButtonSize, ButtonVariant named type exports (clearer than inline CVA expansion)
- AggregateFunction, ColumnAlign type exports from root barrel
...
- GanttChart (L5): interactive timeline with task bars, milestones, dependency arrows, critical path, drag-to-move/resize, group collapse, day/week/month views, today line, hover tooltips
