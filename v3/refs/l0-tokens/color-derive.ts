...
export function deriveDarkPalette(primaryHex: string): DerivedPalette {
  ...
  const accent = hslToHex({
    h,
    s: Math.max(0.55, Math.min(0.85, hsl.s)),
    l: Math.max(0.45, Math.min(0.55, hsl.l)),
  })
  ...
  const bgTertiary = hslToHex({ h, s: ts, l: 0.2 }) // hover/active states
  const surface = hslToHex({ h, s: ts, l: 0.11 }) // = bgSecondary (glass base)
  const surfaceRaised = hslToHex({ h, s: ts, l: 0.2 }) // elevated panels
  ...
  const border = hslToHex({ h, s: ts, l: 0.2 })
...
export function deriveLightPalette(primaryHex: string): DerivedPalette {
  ...
  const accent = hslToHex({
    h,
    s: Math.max(0.6, Math.min(0.9, hsl.s)),
    l: Math.max(0.42, Math.min(0.55, hsl.l)),
  })
  ...
  const fgSecondary = hslToHex({ h, s: 0.03, l: 0.3 })
  const fgMuted = hslToHex({ h, s: 0.03, l: 0.42 })

  // borders
  const border = hslToHex({ h, s: ts, l: 0.9 })
