...
describe('CertBadge', () => {
  it('renders data-component attribute', () => {
    const { container } = render(
      <CertBadge domain="golia.jp" expiresAt="2026-12-01" status="valid" />
    )
    expect(
      container.querySelector('[data-component="cert-badge"]')
    ).not.toBeNull()
  })
  ...
  it('applies status-specific styling', () => {
    const { container } = render(
      <CertBadge domain="old.test" expiresAt="2024-01-01" status="expired" />
    )
