...
describe('FormBuilder', () => {
  it('renders without crash', () => {
    ...
    expect(
      container.querySelector('[data-component="form-builder"]')
    ).not.toBeNull()
  })
  ...
  it('renders number field and calls onChange with number', async () => {
    const user = userEvent.setup()
    const onChange = vi.fn()
    const numFields = [{ id: 'age', label: 'Age', type: 'number' as const }]
  ...
  it('shows required asterisk on text field', () => {
    ...
    const asterisk =
      label.closest('label')?.querySelector('.text-danger') ??
      label.parentElement?.querySelector('.text-danger')
  ...
  it('renders textarea with placeholder', () => {
    ...
    render(
      <FormBuilder fields={taWithPlaceholder} values={{}} onChange={vi.fn()} />
    )
  ...
  it('forwards ref', () => {
    ...
    render(
      <FormBuilder fields={fields} values={{}} onChange={vi.fn()} ref={ref} />
    )
  ...
  it('calls onChange with empty string when number input is cleared', async () => {
    ...
    const numFields = [{ id: 'age', label: 'Age', type: 'number' as const }]
    render(
      <FormBuilder fields={numFields} values={{ age: 5 }} onChange={onChange} />
    )
    const input = screen.getByLabelText('Age')
